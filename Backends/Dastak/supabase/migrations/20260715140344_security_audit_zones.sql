...
create policy dastak_evidence_select_own on storage.objects
for select to authenticated
using (
  bucket_id = 'dastak-evidence'
  and array_length(string_to_array(name, '/'), 1) = 3
  and split_part(name, '/', 1) = 'dastak-partner'
  and split_part(name, '/', 2) = (select auth.uid())::text
  and nullif(split_part(name, '/', 3), '') is not null
);

create policy dastak_evidence_insert_own on storage.objects
...
create policy dastak_evidence_update_own on storage.objects
