...
begin
  ...
  v_distance_terms := private.calculate_merchant_order_distance_terms(
    v_rate,
    v_quote.delivery_distance_m
  );

  if ((v_distance_terms -> 'deliveryFee' ->> 'paise')::integer
      is distinct from v_quote.delivery_fee_paise)
    or ((v_distance_terms -> 'courierPayout' ->> 'paise')::integer
      is distinct from v_quote.courier_payout_paise)
    or (v_rate.merchant_commission_bps
      is distinct from v_quote.merchant_commission_bps)
    or (v_quote.total_paise
      is distinct from v_quote.item_subtotal_paise + v_quote.delivery_fee_paise)
  then
    response_body := pg_catalog.jsonb_build_object(
      'error', pg_catalog.jsonb_build_object(
        'code', 'catalogue_changed',
...
revoke execute on function public.create_merchant_order(uuid, uuid, text, text)
  from public, anon, authenticated;
grant execute on function public.create_merchant_order(uuid, uuid, text, text)
  to service_role;
