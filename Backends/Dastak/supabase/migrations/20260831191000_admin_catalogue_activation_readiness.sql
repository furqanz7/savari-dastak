-- Give Admin clients the same activation truth enforced by the database.
-- Draft catalogue rows may legitimately be unclassified, so categoryTypeId is
-- nullable until the governed taxonomy workflow completes.
...
begin
  ...
  select
    ...
    case when (select pg_catalog.count(*) > v_limit from matched) then (
      select pg_catalog.jsonb_build_object('name', row.canonical_name, 'skuId', row.id)
      from selected row
...
comment on function dastak_v1_api.admin_catalogue_page(
  ...
  'Permission-checked Admin catalogue page with database-authoritative activation readiness.';
