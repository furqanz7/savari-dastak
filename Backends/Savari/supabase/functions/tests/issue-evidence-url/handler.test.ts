...
Deno.test("evidence URL permits an active owner to download another user's Savari driver object", async () => {
...
Deno.test("evidence URL denies an inactive or suspended owner for an owner-only root", async () => {
  ...
  const response = await handleIssueEvidenceUrl(
    request({
      body: {
        bucket: "savari-evidence",
        objectPath: "receipt/44444444-4444-4444-8444-444444444444/receipt.pdf",
        operation: "download",
      },
    }),
    dependencies({
