...
select policies_are('storage', 'objects', array[
  ...
  'savari_evidence_select_own',
  'savari_evidence_update_own'
]);
...
insert into storage.objects (bucket_id, name) values
  ('savari-evidence', 'savari-driver/11111111-1111-4111-8111-111111111111/existing.jpg'),
  ('savari-evidence', 'savari-driver/22222222-2222-4222-8222-222222222222/foreign.jpg');

set local role authenticated;
select set_config('request.jwt.claim.sub', '11111111-1111-4111-8111-111111111111', true);

select results_eq(
  $$select name from storage.objects where bucket_id = 'savari-evidence' order by name$$,
  array['savari-driver/11111111-1111-4111-8111-111111111111/existing.jpg']::text[],
  'authenticated selects only its own exact evidence path'
);
select lives_ok(
  $$insert into storage.objects (bucket_id, name) values ('savari-evidence', 'savari-driver/11111111-1111-4111-8111-111111111111/new.jpg')$$,
  'authenticated inserts its own exact evidence path'
);
select throws_like(
  $$insert into storage.objects (bucket_id, name) values ('savari-evidence', 'savari-driver/22222222-2222-4222-8222-222222222222/denied.jpg')$$,
  '%row-level security%',
  'authenticated cannot insert a foreign evidence path'
);
select throws_like(
  $$insert into storage.objects (bucket_id, name) values ('savari-evidence', 'savari-driver/11111111-1111-4111-8111-111111111111/nested/file.jpg')$$,
  '%row-level security%',
  'authenticated cannot insert a nested evidence path'
);
select throws_like(
  $$insert into storage.objects (bucket_id, name) values ('savari-evidence', 'savari-driver/11111111-1111-4111-8111-111111111111/')$$,
  '%row-level security%',
  'authenticated cannot insert an empty evidence filename'
);
select results_eq(
  $$update storage.objects set metadata = '{"verified":true}'::jsonb where name = 'savari-driver/11111111-1111-4111-8111-111111111111/existing.jpg' returning name$$,
  array['savari-driver/11111111-1111-4111-8111-111111111111/existing.jpg']::text[],
  'authenticated updates its own evidence object'
);
select results_eq(
  $$update storage.objects set metadata = '{"verified":true}'::jsonb where name = 'savari-driver/22222222-2222-4222-8222-222222222222/foreign.jpg' returning name$$,
  array[]::text[],
